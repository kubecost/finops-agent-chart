...
| `global.federatedStorage.fileName`                    | The file name used for the federated storage config secret key                                                                                                                      | `federated-store.yaml` |
| `global.updateCaTrust.enabled`                        | Enable an init container that installs custom CA certificates onto the FinOps Agent pod                                                                                             | `false` |
| `global.updateCaTrust.caCertsSecret`                  | Name of a Secret containing custom CA certificate files to mount into the init container. Mutually exclusive with `caCertsConfig`.                                                  | `ca-certs-secret` |
| `global.updateCaTrust.caCertsConfig`                  | Name of a ConfigMap containing custom CA certificate files to mount into the init container. Mutually exclusive with `caCertsSecret`.                                               | `""` |
| `global.updateCaTrust.caCertsMountPath`               | Path inside the init container where the CA certificate files are mounted. Should not be changed under normal circumstances.                                                              | `/etc/pki/ca-trust/source/anchors` |
| `global.updateCaTrust.resources`                      | Resource requests and limits for the updateCaTrust init container                                                                                                                   | `{}` |
| `global.updateCaTrust.securityContext`                | Security context for the updateCaTrust init container                                                                                                                               | See `values.yaml` |
...
| `agent.collectorDataSource.collectPodAnnotations`    | Collect pod annotations. Every distinct annotation key becomes a label on the collected metric for every pod, so enabling this on a large cluster can significantly increase memory usage. | `false`                                        |
| `agent.collectorDataSource.collectNamespaceAnnotations` | Collect namespace annotations.                                                                                                                                                             | `false`                                        |
...
| `agent.nodes.concurrentPollers`                      | The number of concurrent node pollers used when collecting node stats. When empty, the agent's built-in default is used.                                                                   | `""`                                           |
